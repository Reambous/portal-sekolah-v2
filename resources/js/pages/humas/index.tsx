                    ...
                    <table className="w-full text-left border-collapse table-fixed">
                        <thead>
                            <tr className="bg-gray-900 text-white text-xs uppercase tracking-widest">
                                ...
                                <th className="p-4 border-r border-gray-700 w-2/5">NAMA AGENDA HUMAS</th>
                                <th className="p-4 border-r border-gray-700 w-1/4">EVALUASI / CATATAN</th>
                        ...
                        <tbody>
                            {dataKegiatan.length === 0 ? (
                                <tr><td colSpan={isAdmin ? 7 : 6} className="p-10 text-center text-gray-500 font-bold uppercase tracking-widest border-t-2 border-gray-900">Belum ada arsip jurnal Humas.</td></tr>
                            ) : (
                                    ...
                                    <tr key={item.id} className="border-t-2 border-gray-200 hover:bg-gray-50 transition">
                                        ...
                                        <td className="p-3 text-center">
                                            <div className="flex flex-col gap-1 items-center">
                                                <Link href={`/humas/${item.id}`} className="bg-blue-600 text-white px-2 py-1 text-[10px] font-bold uppercase tracking-wider hover:bg-blue-700 transition w-full text-center">DETAIL</Link>
                                                {isAdmin && (
                                                    <>
                                                        <button onClick={() => router.put(`/humas/${item.id}/status`, { status: item.status === 'pending' ? 'disetujui' : 'pending' })} className={`text-[10px] font-bold uppercase px-2 py-1 w-full text-white ${item.status === 'pending' ? 'bg-green-600' : 'bg-gray-500'}`}>{item.status === 'pending' ? '✔ ACC' : 'BATAL'}</button>
                                                        <button onClick={() => { if(confirm('Hapus item?')) router.delete(`/humas/${item.id}`) }} className="bg-red-600 text-white px-2 py-1 text-[10px] font-bold uppercase w-full">🗑️ HAPUS</button>
                                                    </>
                                                )}
                                            </div>
                                        </td>
