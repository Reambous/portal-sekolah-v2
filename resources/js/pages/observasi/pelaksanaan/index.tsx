...
export default function PelaksanaanIndex({ data }: { data: any }) {
    ...
    const handleDelete = (id: number) => {
        if (confirm('YAKIN INGIN MENGHAPUS OBSERVASI INI?')) {
            router.delete(`/observasi/pelaksanaan/${id}`);
        }
    };

    return (
        <div className="py-8 bg-white min-h-screen font-sans text-gray-900">
            <Head title="Observasi Pelaksanaan" />
            ...
            <div className="max-w-[95%] mx-auto">
                ...
                <div className="mb-6">
                    <Link
                        href="/observasi/pelaksanaan/create"
                        ...
                        <table className="w-full text-left border-collapse table-auto min-w-[900px]">
                            <thead>
                                <tr className="bg-gray-900 text-white text-xs uppercase tracking-widest">
                                    ...
                                    <th className="p-4 border-r border-gray-700 whitespace-nowrap">PEMBERI UMPAN BALIK</th>
                                ...
                                {rows.length === 0 ? (
                                    <tr>
                                        <td colSpan={5} className="p-10 text-center text-gray-500 font-bold uppercase tracking-widest border-t-2 border-gray-900">
                                            Belum ada observasi pelaksanaan.
                                        </td>
                                    </tr>
                                ) : (
                                    rows.map((item: any, index: number) => (
                                        <tr key={item.id} className="border-t-2 border-gray-200 hover:bg-gray-50 transition">
                                            ...
                                            <td className="p-4 border-r-2 border-gray-200 text-xs font-medium text-gray-700">
                                                ...
                                                <div className="text-[10px] text-gray-400 font-bold uppercase">Kelas: {item.kelas_semester}</div>
                                            </td>
                                            <td className="p-4 border-r-2 border-gray-200 text-xs font-bold text-gray-600 uppercase">{item.pemberi_umpan_balik}</td>
                                            <td className="p-3 text-center">
                                                <div className="flex gap-1 justify-center">
                                                    <Link href={`/observasi/pelaksanaan/${item.id}`} className="bg-blue-600 text-white px-3 py-1.5 text-[10px] font-bold uppercase tracking-wider hover:bg-blue-700 transition inline-block">
                                                        DETAIL
                                                    </Link>
...
PelaksanaanIndex.layout = (page: any) => <TopNavLayout>{page}</TopNavLayout>;
