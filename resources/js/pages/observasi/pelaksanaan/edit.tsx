...
export default function PelaksanaanEdit({ observasi, definisi, itemUtama }: { observasi: any; definisi: any; itemUtama: any[] }) {
    const { data, setData, post, processing, errors } = useForm({
        _method: 'PUT',
        hari_tanggal: observasi.hari_tanggal || '',
        nama_guru: observasi.nama_guru || '',
        kelas_semester: observasi.kelas_semester || '',
        mata_pelajaran: observasi.mata_pelajaran || '',
        pemberi_umpan_balik: observasi.pemberi_umpan_balik || '',
        bukti: { ...(observasi.bukti || {}) },
        catatan: { ...(observasi.catatan || {}) },
        refleksi: { ...(observasi.refleksi || { r16: '', r17: '', r18: '' }) },
    });
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        post(`/observasi/pelaksanaan/${observasi.id}`);
    };
    ...
    return (
        <div className="py-8 bg-white min-h-screen font-sans text-gray-900">
            <Head title="Edit Observasi Pelaksanaan" />
...
PelaksanaanEdit.layout = (page: any) => <TopNavLayout>{page}</TopNavLayout>;
