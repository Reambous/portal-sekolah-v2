...
export default function InstrumenCreate({ definisi, totalMaks }: { definisi: any, totalMaks: number }) {
    const collectKodes = () => {
        const kodes: string[] = [];
    ...
    const initialSkor = Object.fromEntries(kodes.map((k) => [k, '']));
    const initialKomentar = Object.fromEntries(kodes.map((k) => [k, '']));
    ...
    return (
        <div className="py-8 bg-white min-h-screen font-sans text-gray-900">
            <Head title="Instrumen Umpan Balik" />
            <div className="max-w-[95%] mx-auto max-w-6xl">
                <div className="border-b-4 border-gray-900 mb-8 pb-4 flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
                    <div>
                        ...
                        <p className="text-gray-500 text-sm font-medium uppercase tracking-wide">Skala 0-4 · 27 indikator · Total maksimal {totalMaks}</p>
                    </div>
                    ...
                    <div className="border-2 border-gray-900 bg-white shadow-sm">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 p-5">
                            <div>
                                ...
                                <input type="text" className={field} placeholder="Contoh: SMP" value={data.jenjang} onChange={(e) => setData('jenjang', e.target.value)} required />
                            </div>
                            <div>
                                <label className="block text-xs font-black uppercase tracking-widest text-gray-700 mb-2">Mata Pelajaran</label>
                                <input type="text" className={field} placeholder="Contoh: IPA" value={data.mata_pelajaran} onChange={(e) => setData('mata_pelajaran', e.target.value)} required />
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <input type="text" className={field} placeholder="Judul perencanaan" value={data.judul_perencanaan} onChange={(e) => setData('judul_perencanaan', e.target.value)} required />
                            </div>
                        </div>
                    </div>
...
InstrumenCreate.layout = (page: any) => <TopNavLayout>{page}</TopNavLayout>;
