...
export default function SarprasIndex({ kegiatan, filters }: { kegiatan: any, filters: any }) {
    ...
    const handleSearch = (e: React.FormEvent) => {
        e.preventDefault();
        router.get('/sarpras', { search: searchTerm }, { preserveState: true, preserveScroll: true });
    };

    return (
        <div className="py-8 bg-white min-h-screen font-sans text-gray-900">
            <Head title="Jurnal Sarpras" />
            <div className="max-w-[95%] mx-auto">
                <div className="border-b-4 border-gray-900 mb-8 pb-4 flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
                    <div>
                        <h2 className="text-3xl font-black text-gray-900 uppercase tracking-tighter mb-1">Jurnal Sarana & Prasarana (Sarpras)</h2>
                        <p className="text-gray-500 text-sm font-medium uppercase tracking-wide">Pencatatan Inventaris, Pemeliharaan Aset, dan Fasilitas Sekolah</p>
                    </div>
                    <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
                        ...
                        <a href="/sarpras/export/csv" className="bg-green-700 text-white px-5 py-2 text-xs font-bold uppercase tracking-wider hover:bg-green-800 border-2 border-green-700 transition">EXCEL / CSV</a>
                        <Link href="/sarpras/create" className="bg-gray-900 text-white px-5 py-2 text-xs font-bold uppercase tracking-wider hover:bg-yellow-500 hover:text-black border-2 border-gray-900 transition shadow-sm">+ CATAT AGENDA</Link>
                    </div>
                </div>
                ...
                {isAdmin && selectedIds.length > 0 && (
                    <div className="mb-4 p-3 bg-red-50 border-2 border-red-600 flex justify-between items-center">
                        ...
                        <button onClick={() => { if(confirm('Hapus massal?')) router.post('/sarpras/bulk-delete', { ids: selectedIds }, { onSuccess: () => setSelectedIds([]) }) }} className="bg-red-600 text-white px-4 py-1.5 text-xs font-black uppercase hover:bg-red-700 transition">❌ HAPUS MASSAL</button>
                    </div>
                )}

              <div className="w-full overflow-x-auto border-4 border-gray-900 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] mb-8">
    {/* KUNCI: table-auto dan min-w-[1000px] agar tabel punya ruang lega di layar HP */}
    <table className="w-full text-left border-collapse table-auto min-w-[1000px]">
        <thead>
            <tr className="bg-gray-900 text-white text-xs uppercase tracking-widest">
                ...
                {/* KUNCI: Semua header wajib diberi whitespace-nowrap agar teks tidak patah baris kebawah */}
                ...
                <th className="p-4 border-r border-gray-700 whitespace-nowrap">NAMA AGENDA SARPRAS</th>
                <th className="p-4 border-r border-gray-700 whitespace-nowrap">EVALUASI / LAPORAN KAPASITAS</th>
            ...
            {dataKegiatan.length === 0 ? (
                <tr>
                    <td colSpan={isAdmin ? 7 : 6} className="p-10 text-center text-gray-500 font-bold uppercase tracking-widest border-t-2 border-gray-900">
                        Belum ada arsip jurnal Sarpras.
                    </td>
                </tr>
            ) : (
                dataKegiatan.map((item: any, index: number) => (
                    <tr key={item.id} className="border-t-2 border-gray-200 hover:bg-gray-50 transition">
                        {isAdmin && (
                            <td className="p-4 border-r-2 border-gray-200 text-center">
                                <input 
                                    ...
                                    onChange={(e) => setSelectedIds(e.target.checked ? [...selectedIds, item.id] : selectedIds.filter(id => id !== item.id))} 
                                />
                            </td>
                        )}
                        <td className="p-4 border-r-2 border-gray-200 text-center font-bold text-xs whitespace-nowrap">{kegiatan.from + index}</td>
                        <td className="p-4 border-r-2 border-gray-200 text-xs font-bold text-gray-600 whitespace-nowrap">
                            {new Date(item.tanggal).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })}
                        </td>
                        
                        {/* KUNCI: Menggunakan max-w-xs dan truncate agar teks isi yang panjang terpotong rapi pakai titik-titik (...) */}
                        ...
                        <td className="p-4 border-r-2 border-gray-200 text-center">
                            {/* KUNCI: Badge status menggunakan inline-block dan whitespace-nowrap agar tidak hancur menjepit teks di HP */}
                        ...
                        <td className="p-3 text-center">
                            <div className="flex flex-col gap-1 items-center w-full">
                                <Link href={`/sarpras/${item.id}`} className="bg-blue-600 text-white px-2 py-1 text-[10px] font-bold uppercase tracking-wider hover:bg-blue-700 transition w-full text-center block">DETAIL</Link>
                                {isAdmin && (
                                    <>
                                        <button onClick={() => router.put(`/sarpras/${item.id}/status`, { status: item.status === 'pending' ? 'disetujui' : 'pending' })} className={`text-[10px] font-bold uppercase px-2 py-1 w-full text-white transition ${item.status === 'pending' ? 'bg-green-600 hover:bg-green-700' : 'bg-gray-500 hover:bg-gray-600'}`}>{item.status === 'pending' ? '✔ ACC' : 'BATAL'}</button>
                                        <button onClick={() => { if(confirm('Hapus item?')) router.delete(`/sarpras/${item.id}`) }} className="bg-red-600 text-white px-2 py-1 text-[10px] font-bold uppercase w-full hover:bg-red-700 transition">🗑️ HAPUS</button>
                                    </>
                                )}
                            </div>
                        </td>
...
SarprasIndex.layout = (page: any) => <TopNavLayout>{page}</TopNavLayout>;