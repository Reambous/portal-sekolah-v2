...
export default function SarprasShow({ kegiatan }: { kegiatan: any }) {
    ...
    return (
        <div className="py-8 bg-white min-h-screen font-sans text-gray-900">
            <Head title="Detail Jurnal Sarpras" />
            <div className="max-w-[95%] mx-auto max-w-4xl">
                <div className="border-b-4 border-gray-900 mb-8 pb-4 flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
                    <div className="w-full md:flex-1 min-w-0">
                        <h2 className="text-3xl font-black text-gray-900 uppercase tracking-tighter mb-1">Detail Jurnal Sarpras</h2>
                    ...
                    <Link href="/sarpras" className="bg-white text-gray-900 border-2 border-gray-900 px-4 py-2 text-xs font-bold uppercase hover:bg-gray-100 transition block text-center shrink-0 w-full sm:w-auto">KEMBALI</Link>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    <div className="space-y-6">
                        <div className="bg-gray-50 border-2 border-gray-900 p-4 shadow-sm space-y-4">
                            ...
                            <div>
                                <h3 className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-0.5">Tanggal Kegiatan</h3>
                        ...
                        {(isAdmin || (isOwner && kegiatan.status === 'pending')) && (
                            <div className="space-y-2">
                                <Link href={`/sarpras/${kegiatan.id}/edit`} className="w-full block text-center bg-yellow-500 text-black border-2 border-gray-900 px-4 py-2 text-xs font-black uppercase tracking-widest hover:bg-yellow-600 transition shadow-sm">📝 EDIT AGENDA</Link>
                                <button onClick={() => { if(confirm('Hapus?')) router.delete(`/sarpras/${kegiatan.id}`) }} className="w-full bg-red-600 text-white border-2 border-gray-900 px-4 py-2 text-xs font-black uppercase tracking-widest hover:bg-red-700 transition shadow-sm">🗑️ HAPUS JURNAL</button>
                            </div>
                        )}
                        {!isAdmin && isOwner && kegiatan.status === 'disetujui' && (
                            <div className="text-[10px] bg-green-50 border border-green-300 text-green-800 p-3 font-bold uppercase text-center">
                    ...
                    <div className="md:col-span-2 space-y-6">
                        <div className="bg-white border-2 border-gray-900 p-6 shadow-sm">
                            <h3 className="text-sm font-black uppercase border-b-2 border-gray-900 pb-2 mb-4">Catatan Log Teknis & Keterangan Fasilitas</h3>
                        ...
                        {kegiatan.lampiran && (
                            <div className="border-2 border-gray-950 p-4 bg-blue-50 flex items-center justify-between gap-4">
                                <div className="truncate"><h5 className="text-xs font-black uppercase text-blue-900">Berkas Pendukung / Nota:</h5><p className="text-xs font-bold text-gray-700 font-mono truncate">{kegiatan.nama_file_asli}</p></div>
...
SarprasShow.layout = (page: any) => <TopNavLayout>{page}</TopNavLayout>;