                            ...
                            >
                                ...
                                <div className="w-full h-40 bg-gray-100 overflow-hidden relative border-b border-gray-200 block">
                                    <Link href={`/berita/${item.id}`} className="block w-full h-full">
                                        {item.gambar ? (() => {
                                            const ekstensi = item.gambar.split('.').pop().toLowerCase();
                                            const urlFile = `/storage/${item.gambar}`;

                                            // A. JIKA FILE ADALAH GAMBAR
                                            ...
                                            // B. JIKA FILE ADALAH PDF
                                            if (ekstensi === 'pdf') {
                                                return (
                                                    <div className="w-full h-full flex flex-col items-center justify-center text-red-600 bg-red-50 p-4 transition group-hover:bg-red-100">
                                                        <svg className="w-12 h-12 mb-1" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                                                            <path strokeLinecap="round" strokeLinejoin="round" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z" />
                                                        </svg>
                                                        <span className="text-[11px] font-black uppercase tracking-wider bg-red-600 text-white px-2 py-0.5">DOKUMEN PDF</span>
                                                    </div>
                                                );
                                            }

                                            // C. JIKA FILE ADALAH WORD (DOC / DOCX)
                                            if (['doc', 'docx'].includes(ekstensi)) {
                                                return (
                                                    <div className="w-full h-full flex flex-col items-center justify-center text-blue-600 bg-blue-50 p-4 transition group-hover:bg-blue-100">
                                                        <svg className="w-12 h-12 mb-1" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                                                            <path strokeLinecap="round" strokeLinejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                                                        </svg>
                                                        <span className="text-[11px] font-black uppercase tracking-wider bg-blue-600 text-white px-2 py-0.5">DOKUMEN WORD</span>
                                                    </div>
                                                );
                                            }
                                            ...
                                            return (
                                                <div className="w-full h-full flex flex-col items-center justify-center text-gray-600 bg-gray-50">
                                                    <span className="text-[11px] font-black uppercase">LAMPIRAN FILE</span>
                                                </div>
                                            );
                                        })() : (
                                            // D. JIKA TIDAK ADA LAMPIRAN SAMA SEKALI
