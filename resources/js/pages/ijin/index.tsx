                    ...
                    <table className="w-full text-left border-collapse table-fixed">
                        <thead>
                            <tr className="bg-gray-900 text-white text-xs uppercase tracking-widest">
                                {isAdmin && <th className="p-4 border-r border-gray-700 w-12 text-center"><input type="checkbox" className="focus:ring-0 border-2 text-gray-900" onChange={(e) => setSelectedIds(e.target.checked ? dataIjin.map((item: any) => item.id) : [])} checked={dataIjin.length > 0 && selectedIds.length === dataIjin.length} /></th>}
                                <th className="p-4 border-r border-gray-700 w-14 text-center">NO</th>
                                <th className="p-4 border-r border-gray-700 w-44">PEGAWAI / TANGGAL</th>
                                <th className="p-4 border-r border-gray-700 w-28 text-center">DURASI JAM</th>
                                <th className="p-4 border-r border-gray-700 w-1/3">ALASAN KETERANGAN</th>
                        ...
                        <tbody>
                            {dataIjin.length === 0 ? (
                                <tr><td colSpan={isAdmin ? 7 : 6} className="p-10 text-center text-gray-500 font-bold uppercase tracking-widest border-t-2 border-gray-900">Belum ada rekaman pengajuan izin.</td></tr>
                            ) : (
                                dataIjin.map((item: any, index: number) => (
                                    <tr key={item.id} className="border-t-2 border-gray-200 hover:bg-gray-50 transition">
                                        ...
                                        <td className="p-4 border-r-2 border-gray-200 text-center font-bold text-xs">{daftarIjin.from + index}</td>
                                        ...
                                        <td className="p-4 border-r-2 border-gray-200 text-center text-xs font-mono font-bold text-blue-900">
                                            {item.jam_mulai && item.jam_selesai ? `${item.jam_mulai.substring(0,5)} - ${item.jam_selesai.substring(0,5)}` : '1 Hari Penuh'}
                                        </td>
                                        <td className="p-4 border-r-2 border-gray-200 text-xs font-medium text-gray-700"><div className="truncate italic" title={item.alasan}>"{item.alasan}"</div></td>
                                        <td className="p-4 border-r-2 border-gray-200 text-center">
                                            <span className={`inline-block px-2 py-0.5 text-[9px] font-black uppercase border-2 ${
                                        ...
                                        <td className="p-3 text-center">
                                            <div className="flex flex-col gap-1 items-center">
                                                <Link href={`/ijin/${item.id}`} className="bg-blue-600 text-white px-2 py-1 text-[10px] font-bold uppercase tracking-wider hover:bg-blue-700 transition w-full text-center">DETAIL</Link>
                                                {isAdmin ? (
                                                    <>
                                                        {item.status === 'pending' ? (
                                                            <div className="grid grid-cols-2 gap-1 w-full">
                                                                <button onClick={() => handleStatus(item.id, 'disetujui')} className="bg-green-600 text-white p-1 text-[9px] font-black uppercase tracking-wider hover:bg-green-700 transition">✔ ACC</button>
                                                                <button onClick={() => handleStatus(item.id, 'ditolak')} className="bg-red-600 text-white p-1 text-[9px] font-black uppercase tracking-wider hover:bg-red-700 transition">✖ TLK</button>
                                                            </div>
                                                        ) : (
                                                            <button onClick={() => handleStatus(item.id, 'pending')} className="bg-gray-500 text-white px-2 py-1 text-[10px] font-bold uppercase tracking-wider hover:bg-gray-600 transition w-full">↩ BATAL</button>
                                                        )}
                                                        <button onClick={() => handleDelete(item.id)} className="bg-red-600 text-white px-2 py-1 text-[10px] font-bold uppercase hover:bg-red-700 transition w-full">🗑️ HAPUS</button>
                                                    </>
                                                ) : (
                                                    item.status === 'pending' ? (
                                                        <div className="flex gap-1 w-full">
                                                            <Link href={`/ijin/${item.id}/edit`} className="bg-yellow-500 text-black px-2 py-1 text-[10px] font-bold uppercase tracking-wider hover:bg-yellow-600 transition flex-1 text-center">EDIT</Link>
                                                            <button onClick={() => handleDelete(item.id)} className="bg-red-600 text-white px-2 py-1 text-[10px] font-bold uppercase hover:bg-red-700 transition flex-1">BATAL</button>
                                                        </div>
                                                    ) : (
                                                        <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">🔒 TERKUNCI</span>
                                                    )
                                                )}
                                            </div>
                                        </td>
